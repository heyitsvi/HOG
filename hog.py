...
def swine_align(player_score, opponent_score):
    ...
    k = 1
    gcd = 1
    while k <= max(player_score,opponent_score):
        if (player_score % k == 0 and opponent_score % k == 0):
            gcd = k
        k+=1
    if (gcd >= 10 and player_score > 0 and opponent_score > 0):
        return True
    else:
        return False
...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    "*** YOUR CODE HERE ***"
    # END PROBLEM 5
    # print("DEBUG: Function Name", say.__name__)
    while(score0 < goal and score1 < goal):
        if(who == 0 and score0 < goal and score1 < goal):
            num_roll0 = strategy0(score0,score1)
            turn_value0 = take_turn(num_roll0, score1, dice)
            score0 += turn_value0

            if (say.__name__ == "echo" or say.__name__ == "say"):
                if (score1==0):
                    say1 = say(score0,score1)
                else:
        ...
        if(who == 1 and score1 < goal and score0 < goal):
            num_roll1 = strategy1(score1,score0)
            turn_value1 = take_turn(num_roll1, score0, dice)
            score1 += turn_value1
...
def announce_highest(who, last_score=0, running_high=0):
    ...
    "*** YOUR CODE HERE ***"
    def say(score0, score1):
        # print('DEBUG: Running High for ',who, running_high)
        if (who == 0  and score0 < GOAL_SCORE):
            if (score0 > last_score and (score0-last_score) > running_high):
                ...
                return announce_highest(0, score0, running_high = (score0-last_score))
                # print("DEBUG: PLayer 0 Running High: ",running_high)
            else:
                return announce_highest(0, score0, running_high)
        if (who == 1 and score1 < GOAL_SCORE):
            if (score1 > last_score and (score1-last_score) > running_high):
                print(abs(score1 - last_score), 'point(s)! The most yet for Player 1')
                # running_high = score1 - last_score
                return announce_highest(1, score1, running_high = (score1 - last_score))
                # print("DEBUG: PLayer 1 Running High: ",running_high)
            else:
...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    roll = 0
    for i in range(1, 11):
        score = make_averaged(roll_dice, 1000)
        score = score(i, dice)
...
def bacon_strategy(score, opponent_score, cutoff=8, num_rolls=6):
    ...
    """
    # BEGIN PROBLEM 10
    # print("DEBUG: Bacon Score: ", free_bacon(opponent_score))
...
def extra_turn_strategy(score, opponent_score, cutoff=8, num_rolls=6):
    ...
    # print("DEBUG:  Bacon Score ", bacon_score)
    # print("DEBUG:  Free Bacon Score ", free_bacon(opponent_score))
    if(extra_turn(bacon_score,opponent_score) == True):
        return 0
    elif (extra_turn(bacon_score,opponent_score) == False and bacon_strategy(score,opponent_score,cutoff,num_rolls) == 0):
        return 0
    else:
        return num_rolls
    # Replace this statement
